...
def infer_image(img: np.ndarray, keypoints: np.ndarray, dust_bin_ids: int,
    ...
    ids_found=torch.Tensor(np.array([0,1,2,3]),device=device).to(torch.int32)
...
def load_models(refinenet_ckpt: Optional[str] = None,device='cuda'):
