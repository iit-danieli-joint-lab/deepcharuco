...
def create_sample(image: np.ndarray, up_factor, true_corners: tuple):
    ...
    #off_x_min = max(-tl, -ref_corner[0] + 96)  # Prevents negative overflow
    #off_x_max = min(tl - 1, image.shape[1] - ref_corner[0] - 96)  # Prevents going out of bounds

    #off_y_min = max(-tl, -ref_corner[1] + 96)
    #off_y_max = min(tl - 1, image.shape[0] - ref_corner[1] - 96)

    #off_x = random.randint(off_x_min, off_x_max)
    #off_y = random.randint(off_y_min, off_y_max)
